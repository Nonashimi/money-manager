...
<template>
  <UCard class="min-w-64 flex-1">
    <template #header>
      <div class="flex items-center justify-between gap-2">
        <span class="flex items-center gap-2 font-medium truncate">
          <span class="size-2 rounded-full shrink-0" :style="{ backgroundColor: accentColor }" />
          {{ jar.name }}
        </span>
    ...
    <div ref="columnEl" :data-jar-id="jar.id" class="space-y-2 min-h-16">
      <div
      ...
      >
        <UIcon name="i-lucide-grip-vertical" class="size-4 text-dimmed mt-0.5 shrink-0" />
        <div class="min-w-0 flex-1">
          <p class="font-medium tabular-nums">{{ formatMoney(card.amount) }}</p>
          <p class="text-sm text-muted truncate">{{ card.description }}</p>
        </div>
        <UButton
          icon="i-lucide-x"
          size="xs"
    ...
    <template #footer>
      <div v-if="isAdding" class="space-y-2">
        <MoneyInput v-model="amount" :min="0" placeholder="Сумма" class="w-full" />
        <UInput v-model="description" placeholder="Описание" class="w-full" @keyup.enter="submitCard" />
        <div class="flex gap-2">
          <UButton size="sm" :loading="submitting" @click="submitCard">Добавить</UButton>
          <UButton size="sm" color="neutral" variant="ghost" @click="isAdding = false">Отмена</UButton>
        </div>
      </div>
      <UButton v-else icon="i-lucide-plus" size="sm" color="neutral" variant="ghost" block @click="isAdding = true">
        Добавить
      </UButton>
    </template>
  </UCard>
