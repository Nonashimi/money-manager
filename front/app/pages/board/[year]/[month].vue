...
<template>
  <UContainer class="py-8 space-y-6">
    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-semibold capitalize">{{ monthLabel }}</h1>
      <div class="flex gap-2">
        <UButton icon="i-lucide-chevron-left" color="neutral" variant="ghost" @click="goToMonth(-1)" />
        <UButton icon="i-lucide-chevron-right" color="neutral" variant="ghost" @click="goToMonth(1)" />
      </div>
    </div>
    ...
    <p v-if="!jarsStore.spendingJars.length" class="text-muted">
      Сначала создайте расходную копилку на странице <NuxtLink to="/jars" class="text-primary">Копилки</NuxtLink>.
    </p>

    <div v-else class="flex gap-4 overflow-x-auto pb-4 snap-x snap-mandatory sm:snap-none -mx-4 px-4 sm:mx-0 sm:px-0">
      <KanbanColumn
